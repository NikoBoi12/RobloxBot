...
logging.basicConfig(level=logging.DEBUG, handlers=[LOG_HANDLER, logging.StreamHandler(sys.stdout)])
...
async def toproller(interaction: discord.Interaction):
    """Displays a paginated leaderboard for the top 'Rolls'."""
    await interaction.response.defer()
    try:
        top_rollers = orderedDataStore.GetLeaderboard("Rolls")
        if not top_rollers or 'orderedDataStoreEntries' not in top_rollers:
             await interaction.followup.send("Could not retrieve leaderboard data for 'Rollers'.")
             return
        ...
        LOGGER.error(f"Error in /toprollers command: {e}", exc_info=True)
...
async def toptippers(interaction: discord.Interaction):
    ...
    await interaction.response.defer()
    try:
        top_tippers = orderedDataStore.GetLeaderboard("Tips")
        if not top_tippers or 'orderedDataStoreEntries' not in top_tippers:
             await interaction.followup.send("Could not retrieve leaderboard data for 'Tippers'.")
             return

        embed = leaderBoards.CreateLeaderEmbed(top_tippers['orderedDataStoreEntries'], "Tippers", 1)
        view = leaderBoards.ButtonView(data_name="Tips", most_recent_pages=top_tippers, leader_pages=[embed], name="Tippers")

        await interaction.followup.send(embed=embed, view=view)
    except Exception as e:
        LOGGER.error(f"Error in /toptippers command: {e}", exc_info=True)
        await interaction.followup.send("An error occurred while fetching the leaderboard.", ephemeral=True)
